document.addEventListener('DOMContentLoaded', function() {
    ...
    messageForm.addEventListener('submit', function(event) {
        ...
        const recipientUsername = document.getElementById('recipientUsername').value; // Получаем имя пользователя из скрытого поля
    ...
    // Запускаем loadNewMessages только после того, как recipientUsername будет установлен
    const recipientUsernameInput = document.getElementById('recipientUsername');
    if (recipientUsernameInput.value) { // Проверяем, установлено ли значение
        loadNewMessages();
    } else {
        // Если recipientUsername еще не установлен, ждем события, которое сигнализирует об его установке
        // Например, вы можете использовать пользовательское событие:
        document.addEventListener('recipientUsernameSet', loadNewMessages); 
    }
