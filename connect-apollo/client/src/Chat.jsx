...
import Cropper from 'react-easy-crop'; // НУЖНО: npm install react-easy-crop
...
function Chat({ socket, username, room, setRoom }) {
    ...
    const sendMessage = async () => {
        ...
        if (attachedFiles.length > 0) {
            ...
            try {
                const response = await fetch('http://localhost:3001/upload-multiple', { method: 'POST', body: formData });
                const data = await response.json();
        ...
        <div className={`main-layout ${isMobile ? 'mobile-mode' : ''}`}>
            ...
            <div className={`left-panel ${isMobile && showMobileChat ? 'hidden' : ''}`}>
                <div className="sidebar-top">
                    ...
                    <button className="fab-btn" onClick={() => setActiveModal('actionMenu')}><svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 24 24"><path fill="#ffffff" d="M13 4v7h7v2h-7v7h-2v-7H4v-2h7V4h2Z"/></svg></button>
                </div>
            ...
            <div className={`right-panel ${isMobile && !showMobileChat ? 'hidden' : ''}`}>
                <div className="glass-chat">
                    ...
                    <div className="chat-body" ref={chatBodyRef} onScroll={handleScroll}>
                        ...
                        {messageList.map((msg, index) => ( <MessageItem key={index} msg={msg} username={username} setImageModalSrc={setImageModalSrc} onDelete={handleDeleteMessage}/> ))}
                    ...
                    <div className="chat-input-wrapper">
                        ...
                        <div className="input-toolbar">
                            <div className="toolbar-left"> <input type="file" style={{ display: 'none' }} multiple ref={fileInputRef} onChange={handleFileSelect} accept="image/*" /> <button className="tool-btn" onClick={() => fileInputRef.current.click()} title="Прикрепить фото">📎</button> </div>
                            <div className="toolbar-right">
                                {currentMessage.trim() || attachedFiles.length > 0 ? ( <button className="send-pill-btn" onClick={sendMessage}> Отправить ↵ </button> ) : ( <button className={`mic-btn ${isRecording ? 'recording' : ''}`} onClick={isRecording ? stopRecording : startRecording}> {isRecording ? formatTime(recordingTime) : '🎤'} </button> )}
                            </div>
                        </div>
                    </div>
                </div>
