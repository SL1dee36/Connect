        ...
        >
            ...
            <div className={`bubble-container ${isLongPress ? 'long-press-active' : ''}`} style={{ transform: `translateX(${translateX}px)`, transition: translateX === 0 ? 'transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)' : 'none' }}>
                <div className={`bubble ${msg.type === 'video' ? 'video-bubble' : ''} ${isTransparentBubble ? 'transparent-bubble' : ''}`}>
                    ...
                    <span className="meta">{msg.time}{isMine && msg.status === 'sent' && <Icons.IconCheck />}</span>
                </div>
            </div>
        </div>
    );
      ...
      <div className={`main-layout ${isMobile ? "mobile-mode" : ""} ${isSelectionMode ? "selection-mode-active" : ""}`} style={{ touchAction: "pan-y" }}>
          ...
          {isSelectionMode ? (
              <div className="sidebar-selection-header">
                  ...
                  <div className="selection-actions">
                       <button className="tool-btn" onClick={handlePinSelected} title="Закрепить/Открепить"><Icons.IconPin /></button>
                       <button className="tool-btn" onClick={() => setActiveModal("addToFolder")} title="В папку"><Icons.IconFolder /></button>
                       <button className="tool-btn" style={{color: '#ff4d4d'}} onClick={handleDeleteSelected} title="Удалить"><Icons.IconTrash /></button>
                  </div>
              </div>
          ) : (
            <p></p>
          ...
          <div className="glass-chat">
            {!room ? (
                <div className="no-chat-selected">
                    <div className="no-chat-icon"><Icons.IconMessage/></div>
             ...
             <>
                ...
                <div className="header-left">
                    {isMobile && (<button className="back-btn" onClick={handleCloseMobileChat}>
                ...
                <div style={{display: 'flex', gap: 10, alignItems: 'center'}}>
                    ...
                    <div style={{ position: "relative" }}>
                        <button className="menu-btn" onClick={() => setShowMenu(!showMenu)}><Icons.IconMenu /></button>
                        {showMenu && (<div className="dropdown-menu"> <div className="menu-item" onClick={openGroupInfo}><Icons.IconInfo /> Информация</div> {!isPrivateChat && (<div className="menu-item" onClick={() => setActiveModal("addToGroup")}><Icons.IconAddToGroup /> Добавить в группу</div>)}</div>)}
                    </div>
                </div>
                </div>
                        ...
                        {recordedMedia ? (
                            <div className="media-preview-bar" style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', gap: 10}}>
                                ...
                                <button className="send-pill-btn" onClick={sendRecordedContent} disabled={isUploading} style={{borderRadius: '50%', width: 45, height: 45, padding: 0, justifyContent: 'center'}}>
                                    {isUploading ? <div className="spinner"></div> : <Icons.IconSend />}
                                </button>
                            </div>
                        ) : (
                            <>
                                ...
                                <div className="input-toolbar">
                                    <div className="toolbar-left" style={{opacity: isRecording ? 0 : 1, pointerEvents: isRecording ? 'none' : 'auto', transition: '0.2s'}}>
                                        ...
                                        <button className="tool-btn" onClick={() => fileInputRef.current.click()} title="Прикрепить фото"><Icons.IconPaperclip></Icons.IconPaperclip></button>
                                        <button 
                                            className="tool-btn" 
        ...
        {activeModal === "notifications" && (
            <Modal title="Уведомления" onClose={() => { setActiveModal(null); setHasUnreadNotifs(false); }}>
                 <div className="settings-list" id="notifications" style={{padding: 0}}>
        ...
        {activeModal === "actionMenu" && (
          <Modal title="NEW MESSAGE" onClose={() => setActiveModal(null)}>
            <div className="action-grid">
              <div className="action-card" onClick={() => setActiveModal("createGroup")}> <span style={{ fontSize: 12}}><Icons.IconNewGroup /></span> <div><div style={{ fontWeight: "bold" }}>Новая группа</div></div> </div>
              <div className="action-card" onClick={() => setActiveModal("searchGroup")}> <span style={{ fontSize: 24 }}><Icons.IconSearchGroup /></span> <div><div style={{ fontWeight: "bold" }}>Найти группу</div></div> </div>
              <div className="action-card" onClick={() => setActiveModal("addFriend")}> <span style={{ fontSize: 24 }}><Icons.IconSearchPeople /></span> <div><div style={{ fontWeight: "bold" }}>Поиск людей</div></div> </div>
              <div className="action-card" onClick={() => setActiveModal("reportBug")}> <span style={{ fontSize: 24 }}><Icons.IconBug/></span> <div><div style={{ fontWeight: "bold" }}>Report Bug</div></div> </div>
              {(username === 'slide36' || myRole === 'admin' || globalRole === 'mod') && (<div className="action-card" onClick={() => { setActiveModal("adminBugs"); fetchBugReports(); }}> <span style={{ fontSize: 24 }}><Icons.IconShield/></span> <div><div style={{ fontWeight: "bold" }}>Admin Bugs</div></div> </div>)}
            </div>
          </Modal>
        )}
        ...
        {activeModal === "adminBugs" && (
            <Modal title="Bug Reports" onClose={() => setActiveModal(null)}>
                <div className="settings-list" id="adminbugs">
