...
app.post("/upload", upload.single("file"), async (req, res) => {
    ...
    try {
        const url = req.file.mimetype.startsWith("image/") ? await processImage(req.file.buffer) : `${BACKEND_URL}/uploads/${Date.now()}.webm`;
        if (!req.file.mimetype.startsWith("image/")) fs.writeFileSync(path.join(uploadDir, path.basename(url)), req.file.buffer);
        res.json({ url });
...
app.post("/upload-multiple", upload.array("files", 10), async (req, res) => {
    ...
    try { const urls = await Promise.all(req.files.map((f) => processImage(f.buffer))); res.json({ urls }); } catch (e) { res.status(500).json({ message: "Upload failed" }); }
});
...
app.post("/report-bug", upload.array("files", 5), async (req, res) => {
    try {
        ...
        let mediaUrls = [];
        if (req.files && req.files.length > 0) mediaUrls = await Promise.all(req.files.map((f) => processImage(f.buffer)));
    ...
    } catch (e) { res.status(500).json({ message: "Failed to report bug" }); }
});

app.get("/bug-reports", async (req, res) => {
    try { if (!db) throw new Error("Database not ready"); res.json(await db.all("SELECT * FROM bug_reports ORDER BY id DESC")); } catch (e) { res.status(500).json({ message: "Error fetching reports" }); }
});
...
io.use((socket, next) => {
    ...
    try { const user = jwt.verify(token, JWT_SECRET); socket.data.username = user.username; next(); } catch (err) { next(new Error("Authentication error")); }
});
...
io.on("connection", async (socket) => {
    ...
    socket.on("create_group", async ({ room }) => { if (!db) return; await db.run(`INSERT INTO group_members (room, username, role) VALUES (?, ?, 'owner')`, [room, username]); socket.emit("group_created", { room }); await pushUpdatesToUser(username); });
    socket.on("join_existing_group", async ({ room }) => { if (!db) return; if (!(await db.get(`SELECT * FROM group_members WHERE room=? AND username=?`, [room, username]))) { await db.run(`INSERT INTO group_members (room, username, role) VALUES (?, ?, 'member')`, [room, username]); } socket.emit("group_joined", { room }); await pushUpdatesToUser(username); });
    socket.on("leave_group", async ({ room }) => { if (!db) return; const role = (await db.get(`SELECT role FROM group_members WHERE room=? AND username=?`, [room, username]))?.role; if (role === "owner" && room !== "General") { await db.run(`DELETE FROM group_members WHERE room=?`, [room]); await db.run(`DELETE FROM messages WHERE room=?`, [room]); io.to(room).emit("group_deleted", { room }); } else { await db.run(`DELETE FROM group_members WHERE room=? AND username=?`, [room, username]); io.to(room).emit("group_info_updated", { members: await db.all(`SELECT * FROM group_members WHERE room=?`, [room]) }); socket.emit("left_group_success", { room }); await pushUpdatesToUser(username); } });
    socket.on("add_group_member", async ({ room, username: targetUser }) => { if (!db) return; if (!(await db.get(`SELECT * FROM group_members WHERE room=? AND username=?`, [room, targetUser]))) { await db.run(`INSERT INTO group_members (room, username, role) VALUES (?, ?, 'member')`, [room, targetUser]); io.to(room).emit("group_info_updated", { members: await db.all(`SELECT * FROM group_members WHERE room=?`, [room]) }); await pushUpdatesToUser(targetUser); } });
    socket.on("remove_group_member", async ({ room, username: targetUser }) => { if (!db) return; await db.run(`DELETE FROM group_members WHERE room=? AND username=?`, [room, targetUser]); io.to(room).emit("group_info_updated", { members: await db.all(`SELECT * FROM group_members WHERE room=?`, [room]) }); await pushUpdatesToUser(targetUser); });
    socket.on("typing", (d) => socket.to(d.room).emit("display_typing", { username: username, room: d.room }));
    socket.on("search_groups", async (q) => { try { if (!db) return socket.emit("search_groups_results", []); const groups = await db.all(`SELECT DISTINCT room FROM group_members WHERE lower(room) LIKE lower(?) LIMIT 20`, [`%${q}%`]); socket.emit("search_groups_results", groups); } catch (e) { socket.emit("search_groups_results", []); } });
    socket.on("search_users", async (q) => { try { if (!db) return socket.emit("search_results", []); const dbUsers = await db.all(`SELECT u.username, p.display_name, p.avatar_url FROM users u LEFT JOIN user_profiles p ON u.username = p.username WHERE u.username LIKE ? OR p.display_name LIKE ? LIMIT 20`, [`%${q}%`, `%${q}%`]); const results = dbUsers.map((user) => { const onlineUser = onlineUsers.find((u) => u.username === user.username); return { username: user.username, display_name: user.display_name || user.username, avatar_url: user.avatar_url, socketId: onlineUser ? onlineUser.socketId : null, isOnline: !!onlineUser, }; }); socket.emit("search_results", results); } catch (e) { socket.emit("search_results", []); } });
    socket.on("get_group_info", async (room) => { if (!db) return; const members = await db.all(`SELECT gm.*, up.avatar_url, up.display_name FROM group_members gm LEFT JOIN user_profiles up ON gm.username = up.username WHERE gm.room=?`, [room]); const myRole = members.find((m) => m.username === username)?.role || "guest"; socket.emit("group_info_data", { room, members, myRole }); });
    socket.on("get_my_profile", async (u) => { if (db) { let data = await db.get(`SELECT * FROM user_profiles WHERE username=?`, [u]); if (!data) { data = { username: u, display_name: u, bio: "", phone: "", avatar_url: "", notifications_enabled: 1 }; await db.run(`INSERT OR IGNORE INTO user_profiles (username, display_name) VALUES (?, ?)`, [u, u]); } if(!data.display_name) data.display_name = u; socket.emit("my_profile_data", data); } });
    socket.on("update_profile", async (d) => { if (!db) return; if (d.newUsername && d.newUsername !== username) { const usernameRegex = /^[a-zA-Z0-9]{3,}$/; if (!usernameRegex.test(d.newUsername)) return socket.emit("error_message", { msg: "Nametag может содержать только латинские буквы (a-Z) и цифры (0-9), минимум 3 символа." }); try { const exists = await db.get("SELECT id FROM users WHERE username = ?", [d.newUsername]); if (exists) return socket.emit("error_message", { msg: "Это имя пользователя (@nametag) уже занято." }); await db.run("BEGIN TRANSACTION"); await db.run("UPDATE users SET username = ? WHERE username = ?", [d.newUsername, username]); await db.run("UPDATE messages SET author = ? WHERE author = ?", [d.newUsername, username]); await db.run("UPDATE messages SET reply_to_author = ? WHERE reply_to_author = ?", [d.newUsername, username]); await db.run("UPDATE friends SET user_1 = ? WHERE user_1 = ?", [d.newUsername, username]); await db.run("UPDATE friends SET user_2 = ? WHERE user_2 = ?", [d.newUsername, username]); await db.run("UPDATE group_members SET username = ? WHERE username = ?", [d.newUsername, username]); await db.run("UPDATE user_profiles SET username = ? WHERE username = ?", [d.newUsername, username]); await db.run("UPDATE blocked_users SET blocker = ? WHERE blocker = ?", [d.newUsername, username]); await db.run("UPDATE blocked_users SET blocked = ? WHERE blocked = ?", [d.newUsername, username]); await db.run("UPDATE user_avatars SET username = ? WHERE username = ?", [d.newUsername, username]); await db.run("UPDATE notifications SET user_to = ? WHERE user_to = ?", [d.newUsername, username]); const involvedRooms = await db.all("SELECT DISTINCT room FROM messages WHERE room LIKE ? OR room LIKE ?", [`${username}_%`, `%_${username}`]); for (const row of involvedRooms) { const parts = row.room.split('_'); if (parts.length === 2 && (parts[0] === username || parts[1] === username)) { const otherUser = parts[0] === username ? parts[1] : parts[0]; const newRoomName = [d.newUsername, otherUser].sort().join('_'); await db.run("UPDATE messages SET room = ? WHERE room = ?", [newRoomName, row.room]); } } await db.run("COMMIT"); socket.emit("force_logout", { msg: "Ваш @nametag изменен. Пожалуйста, войдите снова." }); return; } catch (err) { await db.run("ROLLBACK"); console.error(err); return socket.emit("error_message", { msg: "Ошибка при смене @nametag." }); } } await db.run(`UPDATE user_profiles SET bio=?, phone=?, display_name=?, notifications_enabled=? WHERE username=?`, [d.bio, d.phone, d.display_name, d.notifications_enabled ? 1 : 0, username]); socket.emit("my_profile_data", await db.get(`SELECT * FROM user_profiles WHERE username=?`, [username])); });
    socket.on("get_user_profile", async (u) => { if (!db) return; const p = await db.get(`SELECT * FROM user_profiles WHERE username=?`, [u]); const f = await db.get(`SELECT * FROM friends WHERE (user_1=? AND user_2=?) OR (user_1=? AND user_2=?)`, [username, u, u, username]); socket.emit("user_profile_data", { ...(p || { username: u, display_name: u }), isFriend: !!f }); });
    socket.on("get_avatar_history", async (u) => { if(db) socket.emit("avatar_history_data", await db.all(`SELECT * FROM user_avatars WHERE username=? ORDER BY id DESC`, [u])); });
    socket.on("delete_avatar", async ({ avatarId }) => { if (!db) return; await db.run("DELETE FROM user_avatars WHERE id=?", [avatarId]); const last = await db.get("SELECT * FROM user_avatars WHERE username=? ORDER BY id DESC LIMIT 1", [username]); await db.run("UPDATE user_profiles SET avatar_url=? WHERE username=?", [last?.avatar_url || "", username]); socket.emit("my_profile_data", await db.get(`SELECT * FROM user_profiles WHERE username=?`, [username])); socket.emit("avatar_history_data", await db.all(`SELECT * FROM user_avatars WHERE username=? ORDER BY id DESC`, [username])); });
    socket.on("send_friend_request_by_name", async ({ toUsername }) => { if (!db || toUsername === username) return; const isFriend = await db.get(`SELECT * FROM friends WHERE (user_1=? AND user_2=?) OR (user_1=? AND user_2=?)`, [username, toUsername, toUsername, username]); if(isFriend) return; const exists = await db.get(`SELECT * FROM notifications WHERE user_to=? AND type='friend_request' AND content=?`, [toUsername, username]); if (exists) return; await sendNotification(toUsername, 'friend_request', username); });
    ...
    socket.on("accept_friend_request", async ({ notifId, fromUsername }) => { if (!db || !fromUsername) return; const u1 = fromUsername; const u2 = username; await db.run("INSERT INTO friends (user_1, user_2) VALUES (?, ?)", [u1, u2]); if (notifId) await db.run("DELETE FROM notifications WHERE id=?", [notifId]); await pushUpdatesToUser(u1); await pushUpdatesToUser(u2); await sendNotification(u1, 'info', `${u2} принял вашу заявку`); });
    socket.on("decline_friend_request", async ({ notifId }) => { if (db && notifId) await db.run("DELETE FROM notifications WHERE id=?", [notifId]); });
    socket.on("remove_friend", async (t) => { if (!db) return; await db.run(`DELETE FROM friends WHERE (user_1=? AND user_2=?) OR (user_1=? AND user_2=?)`, [username, t, t, username]); await pushUpdatesToUser(username); await pushUpdatesToUser(t); });
    socket.on("block_user", async (t) => { if (!db) return; await db.run(`INSERT OR IGNORE INTO blocked_users (blocker, blocked) VALUES (?, ?)`, [username, t]); await db.run(`DELETE FROM friends WHERE (user_1=? AND user_2=?) OR (user_1=? AND user_2=?)`, [username, t, t, username]); await pushUpdatesToUser(username); await pushUpdatesToUser(t); });
    socket.on("delete_message", async (id) => { if (!db) return; const msg = await db.get("SELECT * FROM messages WHERE id=?", [id]); if (!msg) return; let isOwner = false; if (!msg.room.includes('_')) { const membership = await db.get("SELECT role FROM group_members WHERE room = ? AND username = ?", [msg.room, username]); if (membership && membership.role === 'owner') { isOwner = true; } } if (msg.author === username || isOwner) { await db.run("DELETE FROM messages WHERE id=?", [id]); io.to(msg.room).emit("message_deleted", id); } });
